...
module ElasticityMixedTensor_mixedBCTests
  ...
  #   const μ = E/(2*(1+ν))
  K = TensorValue(0.25, 0.0, 0.0, 0.04) # this will be variable later...
  Kinv = TensorValue(4.0, 0.0, 0.0, 25.0) # and so will this.
  s_0 = 1
  α = 1
  λ = 1
  μ = 1
  ...
  fex(x) = -(∇⋅σex)(x)
  gex(x) = s_0*pex(x) + α*(∇⋅uex)(x) + (∇⋅zex)(x) # used a mix of the strong form equationss_0 + 
  ...
  Ph_ = TestFESpace(model,reffe_γ,conformity=:L2)
  ...
  Y = MultiFieldFESpace([Sh_,Sh_,Ph_,Vh_,Ph_,Sh_])
  X = MultiFieldFESpace([Sh1,Sh2,Ph,Vh,Gh,Zh])

  # (Cinv∘(σ):τ)
  a1((σ1,σ2),(τ1,τ2)) = ∫(1/(2*μ)*(σ1⋅τ1 + σ2⋅τ2))dΩ -
                        ∫(λ/(2*μ*(2*μ+ d*λ))*(comp1∘σ1+comp2∘σ2)*(comp1∘τ1+comp2∘τ2))dΩ
  
  a2(q,(τ1,τ2)) = ∫(α/(2*μ+ d*λ)*p*(comp1∘τ1+comp2∘τ2))dΩ
  a3(p,q) = (s_0 + d*α^2/(2*μ+ d*λ))*∫(p*q)dΩ

  a((σ1,σ2,p),(τ1,τ2,q)) =  a1((σ1,σ2),(τ1,τ2)) + a2(q,(τ1,τ2)) +  a3(p,q) 
  ...
  function  convergence_test(; nkmax, k=0, generate_output=false)
    ...
    for nk in 1:nkmax
       ...
       error_σ,error_p,error_u, error_γ, error_z, ndofs=solve_elasticityMixed(model; k=k, generate_output=generate_output)
       push!(nn,ndofs)
