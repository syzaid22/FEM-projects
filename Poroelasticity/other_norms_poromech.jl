module PoroelasticityMixedTensor_mixedBCTests
  ...
  reffe_γTen = ReferenceFE(lagrangian,TensorValue{2,2,Float64},k)
  ...
  GTenh = TestFESpace(model,reffe_γTen,conformity=:L2)
  ...
  γten(x) = TensorValue(0.0, γh(x), -γh(x), 0.0) #@. TensorValue(0,γh,-γh,0)
  γhTen = interpolate(γten,GTenh)
  ...
  eph = pex-ph
  euh  = uex-uh
  eγh  = γex - γhTen #comp1∘row2∘γex-γh
  ezh = zex-zh

  error_σ = sqrt(sum(a1((eσ1h,eσ2h),(eσ1h,eσ2h)) + 
                      ∫(1/(2*μ)*((∇⋅eσ1h)*(∇⋅eσ1h)+(∇⋅eσ2h)*(∇⋅eσ2h)))dΩ))
            #sqrt(sum(∫(1/(2*μ)*(eσ1h⋅eσ1h+eσ2h⋅eσ2h))dΩ +
            #         ∫(1/(2*μ)*((∇⋅eσ1h)*(∇⋅eσ1h)+(∇⋅eσ2h)*(∇⋅eσ2h)))dΩ))

  error_p = sqrt(sum(∫((κ + s_0 + d*α^2/(2*μ + d*λ))*(eph*eph))dΩ))
  error_u = sqrt(sum(∫(euh⋅euh)dΩ))
  error_γ = sqrt(sum(∫(eγh⊙eγh)dΩ)) 
  error_z = sqrt(sum(∫(1/κ*(ezh⋅ezh))dΩ + ∫(((∇⋅ezh)*(∇⋅ezh)))dΩ))
